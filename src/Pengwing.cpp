...
//#include "Landschaft/perlin.h"
//#include "Landschaft/terrain.h"
//#include "Landschaft/soil.h"
...
//
////unsigned int VAO;
//std::vector<unsigned int> map_chunks(terrainXChunks* terrainYChunks);
////glGenVertexArrays(1, &VAO);
//
////Shader terrainShader = Shader("ShadowMapping/shadow_mapping.vert", "ShadowMapping/shadow_mapping_unicol.frag");
////setupModelTransformation(terrainShader);
////setupProjectionTransformation(terrainShader, mapWidth, mapHeight); //maybe window width and height, check
//
//
//// Landschaft
...
main(int, char* argv[]) {
    ...
    // Camera ---------------------------------------------------------
    cam = Camera(&scene_mat, "Camera");
    orbitalCam = camera_orbital(window);
    // ----------------------------------------------------------------
...
//void createPlane(std::vector<int>& position, int xOffset, int yOffset, int height, int width, float heightMultiplier, float mapScale, unsigned int& program, unsigned int& plane_VAO) //Check if offsets required
//{
//
...
//    float* expanded_vertices = currTerrain->getTrianglePoints();
//
//    unsigned int vertex_VBO;
//    glGenBuffers(1, &vertex_VBO);
//    glBindBuffer(GL_ARRAY_BUFFER, vertex_VBO);
//    glBufferData(GL_ARRAY_BUFFER, nVertices * 3 * sizeof(float), expanded_vertices, GL_STATIC_DRAW);
//    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
//    glEnableVertexAttribArray(0);
//
//
//    float* expanded_normals = currTerrain->getNormals(height, width);
//
//    unsigned int normal_VBO;
//    glGenBuffers(1, &normal_VBO);
//    glBindBuffer(GL_ARRAY_BUFFER, normal_VBO);
//    glBufferData(GL_ARRAY_BUFFER, nVertices * 3 * sizeof(float), expanded_normals, GL_STATIC_DRAW);
...
//
//
//    float* expanded_vertices2 = currTerrain->getTrianglePoints();
//    float* expanded_colors = new float[nVertices * 3];
//
...
//    glBufferData(GL_ARRAY_BUFFER, nVertices * 3 * sizeof(float), expanded_colors, GL_STATIC_DRAW);
...
//    int currChunkCoordX = (int)round(!useOrbital ? cam.position.x : orbitalCam.position().x / chunkWidth); //nimm aktuelle x-chunk Koordinate
//    int currChunkCoordY = (int)round(!useOrbital ? cam.position.z : orbitalCam.position().z / chunkHeight); //nimm atkuelle y-chunk Koordinate (oder z abhängig von unserem Koord.Sys)
//
//
